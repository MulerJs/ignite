...
public final class BinaryHeapInputStream extends BinaryAbstractInputStream {
    ...
    @Override protected void copyAndShift(Object target, long off, int len) {
        GridUnsafe.getInstance().copyMemory(data, GridUnsafe.getInstance().BYTE_ARR_OFF + pos, target, off, len);
    ...
    @Override protected short readShortFast() {
        ...
        return BIG_ENDIAN ? GridUnsafe.getInstance().getShortLE(data, off) : GridUnsafe.getInstance().getShort(data, off);
    ...
    @Override protected char readCharFast() {
        ...
        return BIG_ENDIAN ? GridUnsafe.getInstance().getCharLE(data, off) : GridUnsafe.getInstance().getChar(data, off);
    }
    ...
    @Override protected int readIntFast() {
        ...
        return BIG_ENDIAN ? GridUnsafe.getInstance().getIntLE(data, off) : GridUnsafe.getInstance().getInt(data, off);
    }
    ...
    @Override protected long readLongFast() {
        ...
        return BIG_ENDIAN ? GridUnsafe.getInstance().getLongLE(data, off) : GridUnsafe.getInstance().getLong(data, off);
    }

    /** {@inheritDoc} */
    @Override protected byte readBytePositioned0(int pos) {
        return GridUnsafe.getInstance().getByte(data, GridUnsafe.getInstance().BYTE_ARR_OFF + pos);
    }
