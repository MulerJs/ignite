...
public class CacheDataTree extends BPlusTree<CacheSearchRow, CacheDataRow> {
    ...
    private int compareKeys(KeyCacheObject key, final long link) throws IgniteCheckedException {
            ...
            try {
                ...
                if (data.nextLink() == 0) {
                    ...
                    for (int i = 0; i < words; i++) {
                        ...
                        long b2 = GridUnsafe.getInstance().getLong(bytes, GridUnsafe.getInstance().BYTE_ARR_OFF + off);
        ...
        for (int i = 0; i < words; i++) {
            int off = GridUnsafe.getInstance().BYTE_ARR_INT_OFF + i * 8;

            long b1 = GridUnsafe.getInstance().getLong(bytes1, off);
            long b2 = GridUnsafe.getInstance().getLong(bytes2, off);
