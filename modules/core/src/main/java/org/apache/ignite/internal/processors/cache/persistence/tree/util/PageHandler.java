...
public abstract class PageHandler<X, R> {
    ...
    public static void copyMemory(ByteBuffer src, long srcOff, ByteBuffer dst, long dstOff, long cnt) {
        ...
        long srcArrOff = src.hasArray() ? src.arrayOffset() + GridUnsafe.getInstance().BYTE_ARR_OFF : 0;
        long dstArrOff = dst.hasArray() ? dst.arrayOffset() + GridUnsafe.getInstance().BYTE_ARR_OFF : 0;

        long srcPtr = src.isDirect() ? GridUnsafe.getInstance().bufferAddress(src) : 0;
        long dstPtr = dst.isDirect() ? GridUnsafe.getInstance().bufferAddress(dst) : 0;

        GridUnsafe.getInstance().copyMemory(srcArr, srcPtr + srcArrOff + srcOff, dstArr, dstPtr + dstArrOff + dstOff, cnt);
    }
    ...
    public static void zeroMemory(ByteBuffer buf, int off, int len) {
        if (buf.isDirect())
            GridUnsafe.getInstance().setMemory(GridUnsafe.getInstance().bufferAddress(buf) + off, len, (byte)0);
    ...
    public static void copyMemory(long srcAddr, long srcOff, long dstAddr, long dstOff, long cnt) {
        GridUnsafe.getInstance().copyMemory(null, srcAddr + srcOff, null, dstAddr + dstOff, cnt);
    }
    ...
    public static void zeroMemory(long addr, int off, int len) {
        GridUnsafe.getInstance().setMemory(addr + off, len, (byte)0);
    }
}
